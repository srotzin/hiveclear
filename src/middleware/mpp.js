/**
 * HiveClear — MPP (Machine Payments Protocol) Middleware
 *
 ...
 *
 * Stream D/E — settlement engine, clearing
...
const PAYMENT_ADDRESS = (
  process.env.HIVECLEAR_PAYMENT_ADDRESS ||
...
// ─── Fee table ───────────────────────────────────────────────
// Stream D/E: settlement engine

const DID_CREDENTIAL_PRICING = {
  '/v1/clear/settle':        0.25,
  '/v1/clear/settle/batch':  1.00,
  '/v1/clear/route':         0.25,
  '/v1/clear/hedge':         0.25,
  '/v1/clear/execute':       0.50,
};

function getMppPrice(path) {
  if (DID_CREDENTIAL_PRICING[path]) return DID_CREDENTIAL_PRICING[path];
  if (path.startsWith('/v1/clear/settle/batch')) return 1.00;
  if (path.startsWith('/v1/clear/settle'))       return 0.25;
  if (path.startsWith('/v1/clear/'))             return 0.25;
  return 0.25;
}
